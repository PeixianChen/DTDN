...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer1 = self._make_layer(block, planes[0], layers[0], groups=groups, norm_layer=norm_layer, IN=False)
        self.layer2 = self._make_layer(block, planes[1], layers[1], stride=2, groups=groups, norm_layer=norm_layer, IN=False)
        self.layer3 = self._make_layer(block, planes[2], layers[2], stride=2, groups=groups, norm_layer=norm_layer, IN=True)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, groups=1, norm_layer=None, IN=False):
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        downsample = None
